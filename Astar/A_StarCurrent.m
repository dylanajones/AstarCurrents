...
% u and v Arrays to store the current velocities
u = ones(10,10) * -1;
v = zeros(10,10);
...
xTarget=9;%X Coordinate of the Target
yTarget=9;%Y Coordinate of the Target
...
OPEN_COUNT=1;
path_cost=0;
goal_distance=hueristic(xNode,yNode,xTarget,yTarget,2);
...
%  plot(xNode+.5,yNode+.5,'go');
 exp_array=expand_array(xNode,yNode,path_cost,xTarget,yTarget,CLOSED,MAX_X,MAX_Y,u,v);
...
figure()
[q_x,q_y] = meshgrid(1:10,1:10);
quiver(q_x,q_y,u,v)

tot_cost = 0;
for i = 1:length(Optimal_path)-1
    tot_cost = tot_cost + move_cost(Optimal_path(i,1),Optimal_path(i,2),Optimal_path(i+1,1),Optimal_path(i+1,2),2,u,v);
end
