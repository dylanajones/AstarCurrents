function exp_array=expand_array(node_x,node_y,hn,xTarget,yTarget,CLOSED,MAX_X,MAX_Y,u,v)
            ...
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    ...
                    if (flag == 1)
                        ...
                        exp_array(exp_count,3) = hn+move_cost(node_x,node_y,s_x,s_y,2,u,v);%cost of travelling to node
                        exp_array(exp_count,4) = hueristic(xTarget,yTarget,s_x,s_y,2);%distance between node and goal
